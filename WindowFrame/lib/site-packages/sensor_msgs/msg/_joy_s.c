...
bool sensor_msgs__msg__joy__convert_from_py(PyObject * _pymsg, void * _ros_message)
{
  ...
  sensor_msgs__msg__Joy * ros_message = _ros_message;
  {  // header
  ...
  {  // axes
    ...
    if (!rosidl_generator_c__float__Sequence__init(&(ros_message->axes), size)) {
  ...
  {  // buttons
    ...
    if (!rosidl_generator_c__int32__Sequence__init(&(ros_message->buttons), size)) {
      PyErr_SetString(PyExc_RuntimeError, "unable to create int32__Sequence ros_message");
      Py_DECREF(seq_field);
      Py_DECREF(field);
      return false;
    }
    int32_t * dest = ros_message->buttons.data;
    for (Py_ssize_t i = 0; i < size; ++i) {
      PyObject * item = PySequence_Fast_GET_ITEM(seq_field, i);
      if (!item) {
        Py_DECREF(seq_field);
...
PyObject * sensor_msgs__msg__joy__convert_to_py(void * raw_ros_message)
{
  ...
  {  // buttons
    ...
    if (ros_message->buttons.size > 0) {
      // populating the array.array using the frombytes method
      PyObject * frombytes = PyObject_GetAttrString(field, "frombytes");
      assert(frombytes != NULL);
      int32_t * src = &(ros_message->buttons.data[0]);
      PyObject * data = PyBytes_FromStringAndSize((const char *)src, ros_message->buttons.size * sizeof(int32_t));
      assert(data != NULL);
      PyObject * ret = PyObject_CallFunctionObjArgs(frombytes, data, NULL);
      Py_DECREF(data);
      Py_DECREF(frombytes);
