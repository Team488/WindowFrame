// generated from rosidl_generator_py/resource/_idl_support.c.em
// with input from lifecycle_msgs:msg\TransitionEvent.idl
// generated code does not contain a copyright notice
#define NPY_NO_DEPRECATED_API NPY_1_7_API_VERSION
#include <Python.h>
#include <stdbool.h>
#ifndef _WIN32
# pragma GCC diagnostic push
# pragma GCC diagnostic ignored "-Wunused-function"
#endif
#include "numpy/ndarrayobject.h"
#ifndef _WIN32
# pragma GCC diagnostic pop
#endif
#include "rosidl_generator_c/visibility_control.h"
#include "lifecycle_msgs/msg/transition_event__struct.h"
#include "lifecycle_msgs/msg/transition_event__functions.h"

bool lifecycle_msgs__msg__transition__convert_from_py(PyObject * _pymsg, void * _ros_message);
PyObject * lifecycle_msgs__msg__transition__convert_to_py(void * raw_ros_message);
bool lifecycle_msgs__msg__state__convert_from_py(PyObject * _pymsg, void * _ros_message);
PyObject * lifecycle_msgs__msg__state__convert_to_py(void * raw_ros_message);
bool lifecycle_msgs__msg__state__convert_from_py(PyObject * _pymsg, void * _ros_message);
PyObject * lifecycle_msgs__msg__state__convert_to_py(void * raw_ros_message);

ROSIDL_GENERATOR_C_EXPORT
bool lifecycle_msgs__msg__transition_event__convert_from_py(PyObject * _pymsg, void * _ros_message)
{
  // check that the passed message is of the expected Python class
  {
    char full_classname_dest[53];
    {
      char * class_name = NULL;
      char * module_name = NULL;
      {
        PyObject * class_attr = PyObject_GetAttrString(_pymsg, "__class__");
        if (class_attr) {
          PyObject * name_attr = PyObject_GetAttrString(class_attr, "__name__");
          if (name_attr) {
            class_name = (char *)PyUnicode_1BYTE_DATA(name_attr);
            Py_DECREF(name_attr);
          }
          PyObject * module_attr = PyObject_GetAttrString(class_attr, "__module__");
          if (module_attr) {
            module_name = (char *)PyUnicode_1BYTE_DATA(module_attr);
            Py_DECREF(module_attr);
          }
          Py_DECREF(class_attr);
        }
      }
      if (!class_name || !module_name) {
        return false;
      }
      snprintf(full_classname_dest, sizeof(full_classname_dest), "%s.%s", module_name, class_name);
    }
    assert(strncmp(
        "lifecycle_msgs.msg._transition_event.TransitionEvent",
        full_classname_dest, 52) == 0);
  }
  lifecycle_msgs__msg__TransitionEvent * ros_message = _ros_message;
  {  // timestamp
    PyObject * field = PyObject_GetAttrString(_pymsg, "timestamp");
    if (!field) {
      return false;
    }
    assert(PyLong_Check(field));
    ros_message->timestamp = PyLong_AsUnsignedLongLong(field);
    Py_DECREF(field);
  }
  {  // transition
    PyObject * field = PyObject_GetAttrString(_pymsg, "transition");
    if (!field) {
      return false;
    }
    if (!lifecycle_msgs__msg__transition__convert_from_py(field, &ros_message->transition)) {
      Py_DECREF(field);
      return false;
    }
    Py_DECREF(field);
  }
  {  // start_state
    PyObject * field = PyObject_GetAttrString(_pymsg, "start_state");
    if (!field) {
      return false;
    }
    if (!lifecycle_msgs__msg__state__convert_from_py(field, &ros_message->start_state)) {
      Py_DECREF(field);
      return false;
    }
    Py_DECREF(field);
  }
  {  // goal_state
    PyObject * field = PyObject_GetAttrString(_pymsg, "goal_state");
    if (!field) {
      return false;
    }
    if (!lifecycle_msgs__msg__state__convert_from_py(field, &ros_message->goal_state)) {
      Py_DECREF(field);
      return false;
    }
    Py_DECREF(field);
  }

  return true;
}

ROSIDL_GENERATOR_C_EXPORT
PyObject * lifecycle_msgs__msg__transition_event__convert_to_py(void * raw_ros_message)
{
  /* NOTE(esteve): Call constructor of TransitionEvent */
  PyObject * _pymessage = NULL;
  {
    PyObject * pymessage_module = PyImport_ImportModule("lifecycle_msgs.msg._transition_event");
    assert(pymessage_module);
    PyObject * pymessage_class = PyObject_GetAttrString(pymessage_module, "TransitionEvent");
    assert(pymessage_class);
    Py_DECREF(pymessage_module);
    _pymessage = PyObject_CallObject(pymessage_class, NULL);
    Py_DECREF(pymessage_class);
    if (!_pymessage) {
      return NULL;
    }
  }
  lifecycle_msgs__msg__TransitionEvent * ros_message = (lifecycle_msgs__msg__TransitionEvent *)raw_ros_message;
  {  // timestamp
    PyObject * field = NULL;
    field = PyLong_FromUnsignedLongLong(ros_message->timestamp);
    {
      int rc = PyObject_SetAttrString(_pymessage, "timestamp", field);
      Py_DECREF(field);
      if (rc) {
        return NULL;
      }
    }
  }
  {  // transition
    PyObject * field = NULL;
    field = lifecycle_msgs__msg__transition__convert_to_py(&ros_message->transition);
    if (!field) {
      return NULL;
    }
    {
      int rc = PyObject_SetAttrString(_pymessage, "transition", field);
      Py_DECREF(field);
      if (rc) {
        return NULL;
      }
    }
  }
  {  // start_state
    PyObject * field = NULL;
    field = lifecycle_msgs__msg__state__convert_to_py(&ros_message->start_state);
    if (!field) {
      return NULL;
    }
    {
      int rc = PyObject_SetAttrString(_pymessage, "start_state", field);
      Py_DECREF(field);
      if (rc) {
        return NULL;
      }
    }
  }
  {  // goal_state
    PyObject * field = NULL;
    field = lifecycle_msgs__msg__state__convert_to_py(&ros_message->goal_state);
    if (!field) {
      return NULL;
    }
    {
      int rc = PyObject_SetAttrString(_pymessage, "goal_state", field);
      Py_DECREF(field);
      if (rc) {
        return NULL;
      }
    }
  }

  // ownership of _pymessage is transferred to the caller
  return _pymessage;
}
